...
Nval=[2 4 8 16 32];
save('gaussian_parameterscubed.mat','berval','Nval');
...
mu3=xval(1:end-1).^3*p;
mu6=xval(1:end-1).^6*p;
...
c1=1-N*perr/(N-1);
c2=perr/(N-1);
[Xnopt]=nonstrategic_quantization_hloop(xval,N,p,yval,var,mu,perr,c1,c2,D1val,mu3)%returns quantization decision levels
Yn=decoder(Xnopt,xval,p,yval,perr,var,mu,c1,c2)%returns quantization representative levels
endistortion=encoderdistortion(Xnopt,Yn,xval,p,yval,perr,var,mu,c1,c2,mu3)+N*c2*(mu6-mu3^2)%encoder distortion
...
save(strcat('gaussian_str_xcubed.mat'),'Xnoptall','Ynall','encdistbern','decdistbern','berval','Nval');
...
function endistortion=encoderdistortion(x,y,xval,p,yval,perr,var,mu,c1,c2,mu3)
...
function [Xnopt]=nonstrategic_quantization_hloop(xval,N,p,yval,var,mu,perr,c1,c2,D1val,mu3)
...
for i=1:size(D1val,1)%iterate over possible (alpha,beta) pairs
    ...
    D1(i)=encoderdistortion([D1val(i,1);D1val(i,2)],q,xval,p,yval,perr,var,mu,c1,c2,mu3);
end
for n=2:N
